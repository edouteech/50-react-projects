                ...
                <Link key={news.title} href={news.url} legacyBehavior>
                    <a target='_blank'>
                        <div className="relative w-full h-[150px] mb-4 group">
                            <Image src={news?.urlToImage != null ? news?.urlToImage : '/img/news-football.webp'} alt='img' fill className='object-cover rounded-md' />
                            <div className='absolute bottom-0 left-0 w-full p-2 z-10 bg-gradient-to-t from-zinc-900 to-transparent'>
                                <p className="font-semiblod text-lg group-hover:text-teal-400 ">{news?.title}</p>
                            </div>
                        </div>
                    </a>
                </Link>
